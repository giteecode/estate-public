<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    ...
    <![endif]-->
            ...
            <body>
                ...
                <nav class="breadcrumb"><i class="Hui-iconfont"></i>报修<a class="btn btn-success radius r"
                        style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);"
                        title="刷新"><i class="Hui-iconfont"></i></a></nav>
                <div class="page-container">

                    <div style="display:inline;margin-top:30px"><span style="width:200px;margin-left:50px;">报修内容</span>
                        <input type="text" class="input-text" style="width:250px;display:inline"
                            placeholder="请输入报修内容进行查询" id="repairContent">
                    </div>
                    ...
                    <div class="cl pd-5 bg-1 bk-gray mt-20">
                        <span class="l" id="globalBtns">
                            <a href="${pageContext.request.contextPath}/user/repair_info/add"
                                class="btn btn-success radius">我要报修</a>
                        </span>
                    ...
                    <table class="table table-border table-bordered table-hover table-bg table-sort" style="">
                        ...
                        <tbody id="repair_info_body">
                        </tbody>
                    </table>

                    <div id="repair_info_bar" class="pagination" style="margin-top:20px;">
                    </div>
                    <div>
                    </div>
                </div>
                ...
                <script type="text/javascript">
                    ...
                    function ajaxList(page) {
                        ...
                        $.ajax({
                            type: 'get',
                            url: "${pageContext.request.contextPath}/user/repair_info/queryList",
                            data: {
                                "page": page,
                            ...
                            success: function (result) {
                                ...
                                for (var i = 0; i < rows.length; i++) {
                                    html += '<tr>';
                                    var repairContentVal = setNullToEmpty(rows[i].repairInfo.repairContent);
                                    html += '<td>' + repairContentVal + '</td>';
                                    var replyContentVal = setNullToEmpty(rows[i].repairInfo.replyContent);
                                    html += '<td>' + replyContentVal + '</td>';
                                    var isDealVal = setNullToEmpty(rows[i].isDealStr);
                                    if (rows[i].repairInfo.isDeal == '1') {
                                        isDealVal = '<span class="badge badge-danger radius">' + isDealVal + '</span>'
                                    }
                                    if (rows[i].repairInfo.isDeal == '2') {
                                    ...
                                    html += '<td>';
                                    if (rows[i].repairInfo.isDeal == '1') {
                                        html += '<a href="${pageContext.request.contextPath}/user/repair_info/update?id=' + rows[i].repairInfo.id + '" class="btn btn-primary  radius btnStyle">修改</a>';
                                    }
                                    if (rows[i].repairInfo.isDeal == '1') {
                                        html += '<a href="javascript:void(0)" onclick="del(this)" data-id="' + rows[i].repairInfo.id + '" class="btn btn-danger  radius btnStyle">删除</a>';
                                    }
                                    html += '</td>';
                                    html += '</tr>';
                                }
